...
public class HeroManager : QSingleton<HeroManager> {
    ...
    public void Init()
    {
        ...
        for (int i = 0; i < curHero.Count; i++)
        {
            ...
            liveHeroList[i].rolePro.SetProValue(RolePro.PRO_CHP, liveHeroList[i].rolePro.tHp);
            liveHeroList[i].SetCurWeapon();
        }
        TemproaryDataUpdate();
        MessageCenter.Instance().AddListener(EventType.SELECTHERO, SelectHeroEvent);
    }

    /// <summary>
    /// 选择Hero事件
    /// </summary>
    public void SelectHeroEvent(MessageEvent e)
    {
        SelectRoleData data = (SelectRoleData)e.Data;
        liveHeroList[data.id].Selected();
    }
    ...
    public void HeroClear()
    {
        ...
        liveHeroList.Clear();
        MessageCenter.Instance().RemoveListener(EventType.SELECTHERO, SelectHeroEvent);
    }
